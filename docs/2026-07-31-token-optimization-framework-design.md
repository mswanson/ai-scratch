...
1. `bmad-hub-setup` (user-level, canonical in `~/.agents/skills`): instantiates `docs/templates/hub-CLAUDE.md` with project specifics (spoke real paths, modules, artifact locations); rebuilds the `.claude/skills` symlink farm and reports drift after BMAD installer runs; verifies hub structure. Layout is ownership-based (§6): on loop repos, `bmad-*` skills are real dirs in every registered CLI tree while custom skills stay symlinked; the skill detects the repo's declared layout and repairs toward it, never blindly re-symlinking.
