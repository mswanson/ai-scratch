...
1. `bmad-hub-setup` (user-level, canonical in `~/.agents/skills`): menu-driven in the bmad-loop-flow style — read-only Verify (drift report) as the default action, Setup as the mutating one. Setup is ONE ordered flow with prerequisites at the front (revised 2026-07-31: the bootstrap chain is part of setup, not a separate path — git and the BMAD install are prerequisites for everything else):
