...
1. `bmad-hub-setup` (user-level, canonical in `~/.agents/skills`): instantiates `docs/templates/hub-CLAUDE.md` with project specifics (spoke real paths, modules, artifact locations); rebuilds the `.claude/skills` symlink farm and reports drift after BMAD installer runs; verifies hub structure. Layout is ownership-based (§6): on loop repos, `bmad-*` skills are real dirs in every registered CLI tree while custom skills stay symlinked; the skill detects the repo's declared layout and repairs toward it, never blindly re-symlinking. Also owns qmd wiring: per-hub collection creation + `context add`, and manual daemon ops (status / restart / stop via the launchctl commands in §5 — `qmd status` can't see the launchd-managed server). Loop-enabled repo path (added 2026-07-31): for repos that will run bmad-loop, walk the bootstrap chain instead of stopping at hub structure — git precondition (never init on the user's behalf), point at `npx bmad-method install` if `_bmad/` is missing (user-run, also lays down bmad-loop-setup), then `bmad-loop-setup` with pinned args (it installs from `main` unless told otherwise — always pass the pin), then auth0-bmad-loop Setup for the flow layer (which now does this delegation itself via its step 0). Two decisions stay human even headless: the phase-branch name and the CLI list.
...
7. Update auth0-bmad-loop for bmad-loop v0.9.0 (§8.4); commit ai-scratch's post-reinstall skill layout per the §6 ownership rule first so worktree runs stop seeing the stale symlink checkout.
