...
1. `bmad-hub-setup` (user-level, canonical in `~/.agents/skills`): instantiates `docs/templates/hub-CLAUDE.md` with project specifics (spoke real paths, modules, artifact locations); rebuilds the `.claude/skills` symlink farm and reports drift after BMAD installer runs; verifies hub structure. Layout is per-repo: bmad-loop repos use real dirs (see §6 exception), so the skill first detects which layout the repo declares and repairs toward that, never blindly re-symlinking.
...
- BMAD installer upgrades can overwrite the symlink dedupe; bmad-hub-setup's drift check is the repair, but only when run. Materialized 2026-07-31 in ai-scratch and resolved the other way: bmad-loop needs real dirs, so that repo dropped the dedupe (§6 exception). Remaining exposure is non-loop repos only.
