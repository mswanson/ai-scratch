...
- Layout note: `.claude/skills/*` were de-symlinked to real dirs on 2026-07-31 (working-tree change on `bmad-loop-dev`, old tracked symlinks show as deleted until committed). `bmad-loop init --force-skills` now works on this layout; the old symlink caveat no longer applies.
